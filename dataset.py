...
class MyDataset:
    ...
    def MyMakeDataSplit(self,dirPath,dataNum,tagName):
        ...
        while True:
            ...
            imgResults = img.copy()
            #cascade = cv2.CascadeClassifier(cascadePath)
            facerect = cascade.detectMultiScale(imgGray,scaleFactor=1.1,minNeighbors=2,minSize=(100,100))
            color = (255,0,255)
            self.__imgTrim = []
            if len(facerect) > 0:
...
if __name__ == "__main__":
    ...
    if isMakeData == "y":
        ...
        pass
    #ネットワークを構築
    net: torch.nn.Module = Net(num=int(num),inputSize=imgSize,Neuron=int(Neuron))
    ...
    num_workers = 0
    #print(num_workers)
    mDataLoader = MyDataLoader(trainRootDir="Resources/train/",testRootDir="Resources/test/",imgSize=imgSize,batchSize=batchSize,num_workers=num_workers)
    #mDataLoader.imshow()
    ...
    #Save
    torch.save(net.state_dict(), ptName)
